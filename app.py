import streamlit as st
import pandas as pd
import requests
import datetime
import math
import io
from reportlab.platypus import SimpleDocTemplate, Paragraph, Spacer
from reportlab.lib.styles import getSampleStyleSheet

# ==========================================
# 1. PAGE CONFIG & STYLING (FIXES UI ISSUES)
# ==========================================
st.set_page_config(page_title="Rubirizi Tax Pro", page_icon="⚖️", layout="wide")

# Custom CSS for the green WhatsApp button and professional cards
st.markdown("""
<style>
    .main {background-color: #f8f9fa;}
    .header {background:#004b23; padding:25px; border-radius:15px; color:white; text-align:center; margin-bottom:20px}
    .card {background:white; padding:30px; border-radius:15px; box-shadow:0 4px 20px rgba(0,0,0,0.08); color:#333; border: 1px solid #eee;}
    .total-tax {color:#d90429; font-size:32px; font-weight:bold; margin:10px 0}
    .btn-wa {
        background-color: #25D366; 
        color: white !important; 
        padding: 15px; 
        border-radius: 10px; 
        text-align: center; 
        display: block; 
        text-decoration: none; 
        font-weight: bold; 
        font-size: 18px; 
        margin-top: 20px;
        box-shadow: 0 4px 10px rgba(0,0,0,0.1);
    }
    .btn-wa:hover {background-color: #128C7E;}
    .row-item {display:flex; justify-content:space-between; padding:12px 0; border-bottom:1px solid #eee; font-size:16px}
</style>
<div class="header">
    <h1>Rubirizi Tax Pro</h1>
    <p>Official 2026 URA Customs & Advisory Portal | Nakawa Branch</p>
</div>
""", unsafe_allow_html=True)

# ==========================================
# 2. VALUATION DATABASE (2026 URA ESTIMATES)
# ==========================================
car_db = {
    "Toyota Fielder (2015-2018)": 6500,
    "Toyota Premio (2014-2017)": 7200,
    "Toyota Harrier (2016-2019)": 14500,
    "Toyota Land Cruiser V8 (2016+)": 45000,
    "Subaru Forester (2015-2018)": 8500,
    "Toyota Rav4 (2014-2017)": 9800,
    "Other / Custom Value": 0
}

# ==========================================
# 3. SIDEBAR & TOOLS
# ==========================================
with st.sidebar:
    st.image("https://www.ura.go.ug/wp-content/uploads/2021/04/URA-Logo.png", width=150)
    st.title("Admin Tools")
    
    def get_live_rate():
        try:
            r = requests.get("https://api.exchangerate-api.com/v4/latest/USD").json()
            return r["rates"]["UGX"]
        except: return 3880 # Standard 2026 rate fallback
    
    rate = st.number_input("Current Exchange Rate (UGX)", value=int(get_live_rate()))
    st.markdown("---")
    st.info("**Consultant:** Victor Tukesiga\n\n**Location:** Nakawa, Kampala")

# ==========================================
# 4. MAIN CALCULATOR INTERFACE
# ==========================================
left, right = st.columns([1, 1], gap="large")

with left:
    st.subheader("📋 Shipment Details")
    cat = st.selectbox("Import Category", ["Motor Vehicle", "General Goods", "Second-hand Clothes (Mivumba)"])
    
    fob_input = 0.0
    if cat == "Motor Vehicle":
        selected_car = st.selectbox("Vehicle Model (URA Value Guide)", list(car_db.keys()))
        if selected_car != "Other / Custom Value":
            fob_input = car_db[selected_car]
            st.success(f"URA Guide FOB: ${fob_input:,.0f}")
        else:
            fob_input = st.number_input("Enter Invoice FOB (USD)", min_value=0.0)
        
        yom = st.number_input("Year of Manufacture", 2008, 2026, 2017)
        cc = st.number_input("Engine Size (cc)", 800, 6000, 1500)
    else:
        fob_input = st.number_input("Item FOB Value", min_value=0.0)
        curr = st.selectbox("Currency", ["USD", "UGX"])

    shipping = st.number_input("Freight / Shipping Cost", min_value=0.0, value=1200.0 if cat == "Motor Vehicle" else 0.0)
    wht_exempt = st.toggle("WHT Exempt (Are you tax compliant?)")

# --- TAX LOGIC ENGINE (2026 STANDARDS) ---
insurance = fob_input * 0.015 # Standard 1.5% Insurance
cif_total = (fob_input + shipping + insurance)
cif_ugx = cif_total * rate if (cat != "Motor Vehicle" or fob_input > 0) else 0

# Base Rates
duty_p, excise_p, env_p, reg_fees = 0.25, 0.10, 0.0, 0.0

if cat == "Motor Vehicle":
    age = 2026 - yom
    if age >= 13: env_p = 0.50
    elif age >= 8: env_p = 0.35
    excise_p = 0.20 if cc > 2500 else 0.10
    reg_fees = 1500000 # Standard registration + Number plates
elif cat == "Second-hand Clothes (Mivumba)":
    env_p = 0.30 # New 2026 Environment Levy
    duty_p = 0.35

# Tax Calculations
i_duty = cif_ugx * duty_p
idf = cif_ugx * 0.015 # 1.5% IDF
infra = cif_ugx * 0.015 # 1.5% Infrastructure
excise = cif_ugx * excise_p
env = cif_ugx * env_p
wht = 0 if wht_exempt else (cif_ugx * 0.06)

# VAT Calculation (Compounded on all duties)
vat_base = cif_ugx + i_duty + excise + idf + infra + env
vat = vat_base * 0.18

total_ura = i_duty + idf + infra + excise + env + vat + wht
final_landing = cif_ugx + total_ura + reg_fees

# ==========================================
# 5. RESULTS & EXPORT (FIXED HTML RENDER)
# ==========================================
with right:
    st.subheader("⚖️ Final Assessment")
    if fob_input > 0:
        # Markdown block for the summary card
        st.markdown(f"""
        <div class="card">
            <p style="margin:0">Total Estimated URA Taxes:</p>
            <div class="total-tax">UGX {math.ceil(total_ura + reg_fees):,.0f}</div>
            <hr>
            <div class="row-item"><span>Import Duty</span><b>{i_duty:,.0f}</b></div>
            <div class="row-item"><span>VAT (18%)</span><b>{vat:,.0f}</b></div>
            <div class="row-item"><span>Environmental Levy</span><b>{env:,.0f}</b></div>
            <div class="row-item"><span>Excise & IDF/Infra</span><b>{(excise + idf + infra):,.0f}</b></div>
            <div class="row-item"><span>Registration Fees</span><b>{reg_fees:,.0f}</b></div>
            <hr>
            <div class="row-item"><strong>TOTAL LANDING COST</strong><strong>UGX {math.ceil(final_landing):,.0f}</strong></div>
            
            <a class="btn-wa" href="https://wa.me/256706631303?text=Hi Victor, I need help clearing my {cat}. Tax estimate: UGX {final_landing:,.0f}" target="_blank">
            💬 Hire Rubirizi Clearing Expert
            </a>
        </div>
        """, unsafe_allow_html=True)
        
        st.write("")
        
        # PDF Generation
        def generate_report():
            buf = io.BytesIO()
            doc = SimpleDocTemplate(buf)
            s = getSampleStyleSheet()
            elements = [
                Paragraph("Rubirizi Tax Pro - Customs Quote", s["Title"]),
                Spacer(1, 12),
                Paragraph(f"Category: {cat}", s["Normal"]),
                Paragraph(f"Total Taxes: UGX {total_ura + reg_fees:,.0f}", s["Heading2"]),
                Paragraph(f"Exchange Rate: {rate} UGX/USD", s["Normal"]),
                Spacer(1, 12),
                Paragraph("Note: This is an estimate for planning. Final valuation is by URA.", s["Italic"])
            ]
            doc.build(elements)
            return buf.getvalue()

        st.download_button("📄 Download PDF Quote", generate_report(), "Rubirizi_Quote.pdf", "application/pdf")
    else:
        st.info("Input item value on the left to see the breakdown.")

st.markdown("---")
st.caption("© 2026 Rubirizi Clearing & Forwarding | Nakawa, Kyinawataka Road | Kampala")
